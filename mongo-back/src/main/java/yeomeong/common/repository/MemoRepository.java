...
public interface MemoRepository extends MongoRepository<Memo, String> {
    ...
    List<Memo> findByTeacherIdAndUpdatedTimeBetweenDates(Long teacherId, LocalDate date);

    // 선생님별 날짜별 메모 중 해당 kid_id를 포함하고 있는 메모 가져오기
    @Query("{ 'teacher_id': ?0,  'date': ?1 , 'kids': ?2 }")
    List<Memo> findByTeacherIdAndUpdatedTimeBetweenDatesAndKidId(Long teacherId, LocalDate date, Long kidId);
}
